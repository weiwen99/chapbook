...
impl ContextWrapper {
    ...
    pub fn new(memory_limit: Option<usize>) -> Result<Self, ContextError> {
        ...
        // chapbook patch: QuickJS 默认 JS 栈 256KB, KaTeX 解析深嵌套公式
        // (如 8 层 \cfrac 连分数) 会 InternalError: stack overflow.
        // 栈限制是 QuickJS 内部调用计数, 不占系统栈, 8MB 无内存代价.
        unsafe {
            q::JS_SetMaxStackSize(runtime, 8 * 1024 * 1024);
        }
